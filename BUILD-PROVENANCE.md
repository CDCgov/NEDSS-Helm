...
The container images referenced by the Helm charts in this repository include
a **SLSA v0.2 provenance attestation** — a machine-readable record of how each
image was built, attached as an OCI referrer. The attestation can be used to
verify that images pulled during a Helm-based deployment were built by the
CI pipeline and haven't been tampered with.
...
`docker pull`, `helm install`, and `kubectl` behave identically regardless of
shape.
